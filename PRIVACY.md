...
`mcp-lever` is a read-only client for [the public job boards companies publish through Lever](https://www.lever.co). It runs on your
...
| ----------------- | ---------------------------------------- |
| `api.lever.co`    | the public job-board API                 |
| `api.eu.lever.co` | the same API for boards hosted in Europe |
...
| A `User-Agent`         | `mcp-lever/<version> (+https://github.com/smeet666/mcp-lever)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                        |

Your requests reach the public job boards companies publish through Lever. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
Open an issue on [the repository](https://github.com/smeet666/mcp-lever/issues). For something exploitable,
...
`mcp-lever` est un client en lecture seule pour [les pages emploi publiques que les entreprises publient via Lever](https://www.lever.co). Il
...
| ----------------- | ---------------------------------------------- |
| `api.lever.co`    | l'API publique des pages emploi                |
| `api.eu.lever.co` | la même API pour les pages hébergées en Europe |
...
| Un `User-Agent`   | `mcp-lever/<version> (+https://github.com/smeet666/mcp-lever)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                             |

Vos requêtes atteignent les pages emploi publiques que les entreprises publient via Lever. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-lever/issues). Pour quelque chose

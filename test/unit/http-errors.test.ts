...
const neverAnswers: typeof fetch = (async (_input: Parameters<typeof fetch>[0], init?: RequestInit) =>
...
describe("la traduction des pannes", () => {
  ...
  it("rend timeout quand la réponse n'arrive pas dans le délai", async () => {
    const code = await codeOfRejection(() => getJson(URL_UNDER_TEST, options(neverAnswers, 5000)));
